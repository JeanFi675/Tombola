...
import { fetchRegles } from '@/lib/nocodb';

const NOCODB_URL = (process.env.NEXT_PUBLIC_NOCODB_URL ?? '').replace(/\/$/, '');
const NOCODB_TOKEN = process.env.NEXT_PUBLIC_NOCODB_TOKEN ?? '';
const REGLES_TABLE_ID = 'm0jr27h7fzynhhu';

async function saveRegles(id: number, value: string) {
  const res = await fetch(`${NOCODB_URL}/api/v2/tables/${REGLES_TABLE_ID}/records`, {
    method: 'PATCH',
...
export default function ReglesPage() {
  ...
  useEffect(() => {
    ...
    fetchRegles()
      .then(record => {
        if (!record) {
          setError('Aucun enregistrement trouvé (Id=1).');
          return;
        }
  ...
  async function handleSave() {
    ...
    setError('');
    try {
      await saveRegles(recordId, draft);
      setContenu(draft);
      setEditing(false);
      setSaved(true);
    } catch (e) {
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="hidden print:block mb-6">
          <h1 className="text-2xl font-black uppercase tracking-tight">Tombola Escalade — Règles &amp; Informations</h1>
